...
class MainController extends Controller {
	...
	public function actionIndex() {
		$this->title = "Гербалайф|Herbalife|Гербалайф Украина";
		$this->meta_desc = "Гербалайф|Herbalife|Гербалайф Украина|Сбалансированное питание Гербалайф|Спортивное питание Гербалайф";
		$this->meta_key = "гербалайф, herbalife, сбалансированное питание, спортивное питание, гербалайф Украина, гербалайф для похудения, диета, как похудеть, правильное питание, витамины";
	...
	public function actionProduct(){
		...
		$product_others = new ProductIcon();
		...
		$product_intro = new ProductIntro();
	...
	public function actionRecipe_Description(){
		...
		$product_others = new ProductIcon("recipes_others_icon");
		$product_others->products = $product_db_others;
		$product_others->auth_user = $this->auth_user;
		$product_others->link_desc = "product";
		$product_others->link_likes = URL::get("likes");
	...
	public function actionTarget(){
		...
		$product_intro = new ProductIntro("target_intro");
		$product_intro->section = $section_db->menu;
		$product_intro->product = $product_db;
		$product_intro->auth_user = $this->auth_user;
	...
	public function actionComplexProg(){
		...
		$complex_intro = new ComplexIntro( $this->auth_user );
	...
	public function actionCart(){
		...
		if( $this->request->addorder ){
			...
			$fields = array( array("delivery", $key), 
							...
							array("address", $this->request->{Config::$AddressDelivery[$this->request->type_delivery]} ), 
			...
			if( $new_order instanceof OrderDB ){ // если обьект создан и его тип соответствует обьекту OrderDB
				...
				$this->mail->send( Config::ORDER_EMAIL, array("order"=>$new_order,"delivery"=>$new_order->delivery,"delivery_price"=>$new_order->delivery_cost, "content_cart"=>$content_cart), "orderMessForSeller" );
				...
				{//в любом случае инф по смс сохраняем в my-errors.log даже если все ОК
					if( !empty($result) ){
						chdir(Config::DIR_ERRORS_LOG);
						$err_str = date('d.m.Y H:i:s' ).PHP_EOL.json_encode($result, JSON_UNESCAPED_UNICODE).PHP_EOL;
						error_log( $err_str, 3, Config::MY_LOG_FILE );
					}
				} 
		...
		$cart_page = new CartPage($this->auth_user,"cart_page_trust-1" );
		...
		$cart_page->address_kiev = $this->request->address_kiev;
		$cart_page->address_dnepr = $this->request->address_dnepr;
	...
	public function actionAddorder(){
		...
		$addorder = new AddOrder( $this->auth_user ); // Формируем новый модуль страницы AddOrder и передаем в конструктор auth_user для расчета параметров заказа
	...
	public function actionRegister() {
		...
		$form->captcha("captcha", "Введите код с картинки:", "<td>", "", "Введите код с картинки:" );
