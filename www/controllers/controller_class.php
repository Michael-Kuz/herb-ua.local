...
abstract class Controller extends AbstractController {
	...
	protected function getHeader() {
		...
		$header->meta("author", false, "Михаил Кузьменко", false);//для herbal24.com.ua
		$header->meta("yandex-verification", false, "20723929a1fbc328",false);
		$header->meta(false, "og:type","website" , false);
		...
		$header->css = array("/styles/main.css?v=1", "/styles/prettify.css","/styles/animate.css");
	...
	protected function ConsultationFolder(){
		...
		$consultation = new ReviewsMenu("products_list");
		$consultation->title = "КОРЗИНА";
	...
	protected function getDeliveryCost( $delivery_type ){
		foreach( Config::$deliveries[$delivery_type] as $key => $value );
		$delivery_price = 0;
		if( isset($_SESSION["ForFreeDelivery"]) )
			if( $_SESSION["ForFreeDelivery"] == 0 )
				switch( $delivery_type ){
					case "self_kiev":  //=== индекс для com.ua
					case "self_dnepr": //=== индекс для com.ua
					case "delivery":   //=== индекс для com.ua
						$delivery_price = 0;
						break;
					default:
						$delivery_price = 0;
