...
$(document).ready( function() {
			...
			if( el_input[i].checked ){
				switch( el_input[i].value ){
					case "self":
						...
						break;
					case "self_kiev": //=== для com.ua
						$(".cour_post_mail, .self, .self_dnepr").css("display", "none");
						$(".cour_post_mail textarea[name='address']").removeAttr("required");
						$(".self_kiev").css("display", "block");
						break;
					case "self_dnepr": //=== для com.ua
						$(".cour_post_mail, .self_kiev").css("display", "none");
						$(".cour_post_mail textarea[name='address']").removeAttr("required");
						$(".self_dnepr").css("display", "block");
						break;	
					case "courier":
					case "postamat":
					case "mail":
					case "delivery": //=== для com.ua
						$(".self, .self_kiev, .self_dnepr").css("display", "none");
	...
	$("#cart_trust-1 input[type='radio']").bind("click", function(){
		switch( $(this).val() ){
			case "self":
				$(".cour_post_mail, .self_kiev, .self_dnepr").css("display", "none");
				$(".cour_post_mail textarea[name='address']").removeAttr("required");
				$(".self").css("display", "block");
				$("#cart_trust-1 input[name='city']").attr("required","");
				break;
			case "self_kiev": //=== для com.ua
				...
				break;
			case "self_dnepr": //=== для com.ua
				$(".cour_post_mail, .self, .self_kiev").css("display", "none");
				$(".cour_post_mail textarea[name='address']").removeAttr("required");
				$(".self_dnepr").css("display", "block");
				break;	
			case "courier":
	...
	function DeliveryPrice( obj ){ /*=== корректировка цены доставки ===*/
		...
		for (var i = 0; i < el_p.length; i++) {
			...
			switch( i ){
				case 2:
					el[0].innerHTML = obj.for_free_delivery ? obj.delivery_price+" грн." : "Бесплатная";
			}
		}
	}
	...
	function ConditionsForFreeDelivery( obj ){ /*=== вывод инфы по условию бесплатной доставки ===*/
		...
		if( obj.for_free_delivery == obj.summ_for_free_delivery ) { 
			$("#cart_trust-1 .free_delivery").html( "Чтобы получить бесплатную доставку, необходимо сделать заказ на сумму не менее <b>"+obj.for_free_delivery+" "+obj.suffix+"."+"</b> (данная акция не распространяется на протеиновые батончики)." );
		} else if( obj.for_free_delivery > 0 && obj.for_free_delivery < obj.summ_for_free_delivery ) { 
			$("#cart_trust-1 .free_delivery").html( "Чтобы получить бесплатную доставку, необходимо добавить в заказ продукты на сумму  <b>"+obj.for_free_delivery+" "+obj.suffix+"."+"</b> (данная акция не распространяется на протеиновые батончики)." );
		} else if( obj.for_free_delivery == 0 ) { 
			$("#cart_trust-1 .free_delivery").html( "Вы можете воспользоваться бесплатной доставкой." );
	...
	$(".rub, .hryvnia").bind("mouseover",function(event){
		showMessage('Чтобы увидеть цену, введите Ваш логин и пароль,<br/>чтобы получить логин и пароль зарегистрируйтесь',event);	
	});
	$(".rub, .hryvnia").bind("mouseout",function(event){
		$("#interactive_message").hide();	
	});
	/*=== функция открывающая окно формы обратного звонка ===*/
	$(".butt_call_back").bind("click",function(event){
		$("#call_back").show(100);	
	});
